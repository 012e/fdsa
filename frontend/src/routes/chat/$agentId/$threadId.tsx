...
function AssistantUIDemo() {
  const { agentId, threadId } = Route.useParams()
  const search = Route.useSearch()
  const navigate = useNavigate({ from: Route.fullPath })
  const isNewThread = search.new === 'true'
  ...
  useEffect(() => {
    if (memory?.result && !threadId) {
      navigate({ to: newThreadLink('chat', agentId) as any })
    }
...
const CustomRuntimeProvider = ({
  ...
  threadId,
  refreshThreadList,
}: { children: React.ReactNode } & Omit<ChatProps, 'memory' | 'agentName' | 'isNewThread'>) => {
  const {
    messages,
    sendMessage,
    cancelRun,
    ...
    <ol className="aui-root aui-thread-list-root flex flex-col items-stretch gap-1.5">
      <li>
        <Button
          ...
          variant="outline"
          onClick={() => navigate({ to: newThreadLink('chat', agentId) as any })}
        >
          <PlusIcon />
          New Thread
        </Button>
        ...
        return (
          <li key={thread.id} className="flex items-center gap-2 w-full">
            <Button
              ...
              variant="ghost"
              onClick={() =>
                navigate({ to: `/chat/${agentId}/${thread.id}` as any })
              }
