...
export class OpenSearchVector extends MastraVector<OpenSearchVectorFilter> {
  ...
  async describeIndex({ indexName }: DescribeIndexParams): Promise<IndexStats> {
    const { body: indexInfo } = await this.client.indices.get({
      index: indexName,
    });
    const mappings = indexInfo[indexName]?.mappings;
    const embedding: any = mappings?.properties?.embedding;
    const spaceType = embedding.method
      .space_type as keyof typeof REVERSE_METRIC_MAPPING;
  ...
  async query({
    ...
    try {
      ...
      const response = await this.client.search({
        ...
        body: {
          query: {
            knn: { embedding: { vector: queryVector, k: topK } },
          },
          ext: {
            mmr: {
              diversity: 0.7,
   ...
   */
  async updateVector(
    params: UpdateVectorParams<OpenSearchVectorFilter>
  ): Promise<void> {
  ...
  private async updateVectorById(
    ...
    try {
      ...
      const { body } = await this.client
        ...
        .catch(() => {
          throw new Error(
            `Document with ID ${id} not found in index ${indexName}`
          );
        });

      if (!body || !body._source) {
        throw new Error(
          `Document with ID ${id} has no source data in index ${indexName}`
        );
      }
      existingDoc = body;
    } catch (error) {
  ...
  async deleteVectors({
    ...
    filter,
    ids,
  }: DeleteVectorsParams<OpenSearchVectorFilter>): Promise<void> {
    // Validate mutually exclusive parameters
    if (ids && filter) {
      throw new MastraError({
        id: createVectorErrorId(
