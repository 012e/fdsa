...
class TemporalContainer(DockerContainer):
    """Custom Temporal container using the official Temporal image."""
    
    def __init__(self, image: str = "temporalio/temporal:1.5.1", **kwargs):
        super().__init__(image, **kwargs)
        self.with_exposed_ports(7233, 8233)
        self.with_command([
            "server", "start-dev", 
            "--ip", "0.0.0.0"
        ])
        # Set environment to ensure proper startup
        self.with_env("TEMPORAL_CLI_ADDRESS", "localhost:7233")
    
    def start(self):
        """Start the Temporal container and wait for it to be ready."""
        super().start()
        # Wait for Temporal to be ready
        wait_for_logs(self, "Temporal server started", timeout=60)
        return self
    
    def get_connection_url(self) -> str:
        """Get the connection URL for Temporal."""
...
def temporal_container():
    """Start Temporal container for testing."""
    container = TemporalContainer()
...
@pytest.fixture(scope="module")
def temporal_client(temporal_container):
    """Create Temporal client connected to the test container."""
    async def _create_client():
        connection_url = temporal_container.get_connection_url()
...
async def test_snippet_ingestion_workflow(
    ...
    try:
        ...
        # Get the document
        doc_response = opensearch_client.get(
            index="test_code_snippets",
            id=SNIPPET_ID
        )
        ...
        for i, chunk in enumerate(chunks):
            ...
            print(f"  ✓ Chunk {i}: {len(chunk['code'])} chars code, "
        ...
        print(f"  ✓ Document searchable (found {search_response['hits']['total']['value']} results)")
...
async def test_temporal_connection(temporal_client):
    """Test that Temporal container is properly connected."""
    # Simply verify we can interact with the client
    assert temporal_client is not None
    print(f"\nTemporal client connected: {temporal_client.service_client.config.target_host}")
